...
<section class="pt-24 pb-10 px-4 bg-[#0A0612]">
    <div class="max-w-6xl mx-auto">
        <nav class="flex items-center gap-1.5 text-xs text-zinc-500 mb-5" aria-label="Breadcrumb">
            ...
            <span class="text-zinc-400">Tag: {{ $tag->name }}</span>
        </nav>
        <div class="max-w-2xl">
            <p class="text-xs font-bold text-violet-400 uppercase tracking-widest mb-3">Tag</p>
            <h1 class="text-3xl sm:text-4xl font-bold text-white mb-3">
                <span class="text-zinc-500 font-normal">#</span>{{ $tag->name }}
            </h1>
            <p class="text-zinc-400 text-base">
                {{ $posts->total() }} {{ $posts->total() === 1 ? 'artigo' : 'artigos' }} com esta tag.
            </p>
        </div>
    </div>
</section>
            ...
            <div>
                @if($posts->isEmpty())
                    <div class="flex flex-col items-center justify-center py-24 text-center">
                        <p class="text-zinc-500 mb-4">Nenhum artigo com esta tag ainda.</p>
                        <a href="{{ route('blog.index') }}"
