...
@section('title', ($post->meta_title ?: $post->title) . ' | Blog 99web')
...
<script type="application/ld+json">
{
    ...
    "author": {
        "@type": "Person",
        "name": "{{ addslashes($post->author->name ?? '99web') }}"
    },
    "publisher": {
        "@type": "Organization",
        "name": "99web",
...
<script type="application/ld+json">
{
    ...
    "itemListElement": [
        ...
        {"@type": "ListItem", "position": 3, "name": "{{ addslashes($post->category->name ?? '') }}", "item": "{{ route('blog.category', $post->category->slug ?? '') }}"},
        {"@type": "ListItem", "position": 4, "name": "{{ addslashes($post->title) }}", "item": "{{ route('blog.show', $post->slug) }}"}
    ]
}
</script>
@endpush
