...
@section('title', 'Contacto — ' . $contact->name)
@section('breadcrumb', 'Contacto #' . $contact->id)
...
<div class="space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <span class="text-sm text-white">#{{ $contact->id }} — {{ $contact->name }}</span>
    </div>
    ...
    <div class="grid lg:grid-cols-[1fr_300px] gap-5 items-start">
        ...
        <div class="space-y-5">
            ...
            {{-- Dados do contacto --}}
            <x-admin.card title="Dados do contacto">
                <div class="p-6 grid sm:grid-cols-2 gap-5">
                    ...
                    @foreach([
                        ...
                        <div>
                            ...
                            @elseif($label === 'Website' && $value !== '—')
                                <a href="{{ $value }}" target="_blank" class="text-sm text-violet-400 hover:text-violet-300 transition-colors break-all">{{ $value }}</a>
                            @else
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white">{{ $contact->created_at->format('d/m/Y \à\s H:i') }}</p>
                    </div>

                    @if($contact->responded_at)
                        <div>
                            <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Respondido em</p>
                            <p class="text-sm text-emerald-400">{{ $contact->responded_at->format('d/m/Y \à\s H:i') }}</p>
                        </div>
                    @endif
            ...
            {{-- Mensagem --}}
            <x-admin.card title="Mensagem">
                <div class="p-6">
                    <p class="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap">{{ $contact->message }}</p>
                </div>
            </x-admin.card>
            ...
            <x-admin.card title="Notas internas">
                <div
                    class="p-6"
                    x-data="{
                        notes: @js($contact->notes ?? ''),
                        saving: false,
                        saved: false,
                        async save() {
                            this.saving = true;
                            const r = await fetch('{{ route('admin.contactos.updateNotes', $contact) }}', {
                                method: 'PATCH',
                                headers: {
        ...
        <div class="space-y-4">
            ...
            <x-admin.card title="Estado">
                <div class="p-5 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <x-admin.status-badge :status="$contact->status" type="contact"/>
                    </div>

                    {{-- Alterar status --}}
                    <form method="POST" action="{{ route('admin.contactos.updateStatus', $contact) }}">
                        @csrf @method('PATCH')
                        ...
                        >
                            @foreach(['novo' => 'Novo', 'em_analise' => 'Em análise', 'respondido' => 'Respondido', 'fechado' => 'Fechado'] as $val => $label)
                                <option value="{{ $val }}" @selected($contact->status === $val)>{{ $label }}</option>
                            @endforeach
                        </select>
                        <button
                            type="submit"
            ...
            <x-admin.card title="Ações">
                <div class="p-4 space-y-2">
                    ...
                    <a
                        href="mailto:{{ $contact->email }}?subject=Re: Contacto via 99web"
                    ...
                    <form
                        method="POST"
                        action="{{ route('admin.contactos.destroy', $contact) }}"
                        x-data
                        @submit.prevent="if(confirm('Eliminar este contacto permanentemente?')) $el.submit()"
                    >
                        @csrf @method('DELETE')
                        <button
                            type="submit"
            ...
            <x-admin.card>
                <div class="p-4 space-y-2.5 text-xs text-zinc-500">
                    <div class="flex justify-between">
                        <span>ID</span>
                        <span class="text-zinc-300">#{{ $contact->id }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Criado</span>
                        <span class="text-zinc-300">{{ $contact->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    @if($contact->updated_at && $contact->updated_at != $contact->created_at)
                        <div class="flex justify-between">
                            <span>Atualizado</span>
                            <span class="text-zinc-300">{{ $contact->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    @endif
                </div>
            </x-admin.card>
