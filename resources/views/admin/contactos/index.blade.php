...
<div class="space-y-5">
    ...
    {{-- Filtros --}}
    <x-admin.filter-bar :action="route('admin.contactos.index')">
        ...
        {{-- Source --}}
        <div class="min-w-[140px]">
            <label class="block text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Origem</label>
            <select
                name="source"
                class="w-full px-3 py-2 rounded-lg bg-[#0F0A1A] border border-zinc-700 text-sm text-white
                       focus:outline-none focus:border-violet-500/60 transition-colors appearance-none"
    ...
    <x-admin.table>
        ...
        <x-slot:header>
            ...
            <span class="text-xs text-zinc-500">{{ $contacts->count() }} nesta página</span>
        </x-slot:header>

        <thead>
            <tr class="border-b border-white/5">
                <th class="text-left px-6 py-3 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Nome</th>
                <th class="text-left px-4 py-3 text-[10px] font-bold text-zinc-500 uppercase tracking-wider hidden md:table-cell">Telefone</th>
                <th class="text-left px-4 py-3 text-[10px] font-bold text-zinc-500 uppercase tracking-wider hidden lg:table-cell">Origem</th>
        ...
        <tbody class="divide-y divide-white/[0.04]">
            ...
            <tr class="hover:bg-white/[0.02] transition-colors group">
                <td class="px-6 py-4">
                    ...
                    <div class="text-xs text-zinc-500 mt-0.5">{{ $contact->email }}</div>
                </td>
                <td class="px-4 py-4 hidden md:table-cell">
                    <span class="text-sm text-zinc-400">{{ $contact->phone ?? '—' }}</span>
                </td>
                <td class="px-4 py-4 hidden lg:table-cell">
                    <span class="text-[11px] font-medium px-2 py-0.5 rounded bg-zinc-800 text-zinc-400">
                        {{ $contact->source ?? '—' }}
                    </span>
                </td>
                <td class="px-4 py-4">
                ...
                <td class="px-4 py-4 hidden sm:table-cell">
                    <span class="text-xs text-zinc-500" title="{{ $contact->created_at->format('d/m/Y H:i') }}">
                ...
                <td class="px-4 py-4">
                    <div class="flex items-center justify-end gap-2">
                        ...
                        {{-- Ver --}}
                        <a
                            href="{{ route('admin.contactos.show', $contact) }}"
                            class="w-7 h-7 rounded-md flex items-center justify-center text-zinc-500
                        ...
                        <form
                            ...
                            x-data
                            @submit.prevent="if(confirm('Eliminar este contacto?')) $el.submit()"
                        >
                            @csrf @method('DELETE')
                            <button
                                type="submit"
        ...
        <x-slot:footer>
            <x-admin.pagination :items="$contacts"/>
        </x-slot:footer>
